...
BELOWGROUND_COMPARTMENTS = [c for c in BIOMASS_COMPARTMENTS if c not in ABOVEGROUND_COMPARTMENTS]
...
def _fill_missing_gc2f_units(cft_ds: xr.Dataset, var_list: List[str], e: Exception):
    """
    ...
    if (
        any_gc2f
        and GC2F_UNITS_SOURCE_VAR in cft_ds
        and "units" in cft_ds[GC2F_UNITS_SOURCE_VAR].attrs
    ):
        units = cft_ds[GC2F_UNITS_SOURCE_VAR].attrs["units"]
        print(f"{e}; assuming {units} based on {GC2F_UNITS_SOURCE_VAR}")
        for v in var_list:
            if GC2F in v:
                cft_ds[v].attrs["units"] = units
                cft_ds[v].attrs["units_source"] = GC2F_UNITS_SOURCE_VAR
        assert all("units" in cft_ds[v].attrs for v in var_list), e
    else:
        raise e
    return cft_ds


def _get_das_to_combine(cft_ds: xr.Dataset, case_name: str, var_list: List[str]):
    # Every variable should have a unit attribute
    msg = f"{case_name}: Not every variable in list has units: {var_list}"
    try:
        assert all("units" in cft_ds[v].attrs for v in var_list), msg
    except AssertionError as e:
        case = _fill_missing_gc2f_units(cft_ds, var_list, e)

    # Units should be the same for all variables
    unit_set = {cft_ds[v].attrs["units"] for v in var_list}
    msg = f"Multiple units ({unit_set}) found in these variables: {var_list}"
    assert len(unit_set) == 1, msg
    units = cft_ds[var_list[0]].attrs["units"]
...
def _get_case_abovebelowground_biomass(cft_ds: xr.Dataset, case_name: str):
    for a_or_b in ["above", "below"]:
        ...
        for phase_transition in PHASE_TRANSITIONS:
            ...
            var_list = [f"{c}C_AT_{phase_transition.upper()}_PERHARV" for c in compartment_list]
            if not all(v in cft_ds for v in var_list):
                continue
            var_list_list.append(var_list)

            # Get variables to sum and their units
            units, da = _get_das_to_combine(cft_ds, case_name, var_list)

            # Mask where there was no harvest
            da = da.where(cft_ds["HARVEST_REASON_PERHARV"] > 0)

            # Sum them
            da = da.sum(dim="variable", keep_attrs=True)

            cft_ds[out_var] = da.mean(dim="mxharvests", keep_attrs=True)
            cft_ds[out_var].attrs["units"] = units
            ...
            cft_ds[out_var_crop].attrs["units"] = cft_ds[out_var].attrs["units"]

        # Nonsense check: Expected (relative) number of negatives at each phase
        _check_nonsense_neg_at_valid_harv(cft_ds, var_list_list)

    return cft_ds


def _check_nonsense_neg_at_valid_harv(cft_ds: xr.Dataset, var_list_list: List[List[str]]):
    """
    ...
    do_check = (
        ...
        and valid_harvest_var in cft_ds
    )
    if not do_check:
        return
    err_msg_list = []
...
def _get_case_grainc_at_maturity(cft_ds, case_name):
    ...
    if len(missing_products) == len(product_list):
        print(f"{case_name}: All grain C product variables missing: {missing_vars}")
        return cft_ds, out_var
    ...
    if missing_products:
        ...
        print(
            f"{case_name}: Missing grain C outputs for {missing_products}; including only {present_products}",
        )

    # Get variables to sum and their units
    _, da = _get_das_to_combine(cft_ds, case_name, var_list)

    # Mask not-mature-enough harvests
    mask_var = f"{maturity_level}_HARVEST"
    da = da.where(cft_ds[mask_var])

    # Get grain C at maturity for each CFT
    assert not np.any(da < 0), f"Unexpected negative value(s) in variables {var_list}"
    cft_ds[out_var] = da.mean(dim="mxharvests", keep_attrs=True).sum(
        dim="variable",
        keep_attrs=True,
    )

    assert "units" in cft_ds[out_var].attrs
    return cft_ds, out_var


def get_crop_biomass_vars(cft_ds, case_name):
    """
    Get crop biomass variables for a case
    """
    case = _get_case_max_lai(cft_ds)
    case = _get_case_abovebelowground_biomass(cft_ds, case_name)

    # Get grain C at maturity
    case, var = _get_case_grainc_at_maturity(cft_ds, case_name)
    # Combine CFTs to crops, if variable not already present
    var_crop = var + "_crop"
    if var_crop not in cft_ds:
        cft_ds = combine_cft_to_crop(
            cft_ds,
            var,
            var_crop,
...
def get_caselist_crop_biomass_vars(case_list):
    ...
    for case in case_list:
        case.cft_ds = get_crop_biomass_vars(case.cft_ds, case.name)
